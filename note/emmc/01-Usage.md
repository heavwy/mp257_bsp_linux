...
>
> **字数：** 中文字数 10,103 + 英文单词 4,803 ≈ **14,906 字**，**行数：1,943**  
> **建议阅读时间：** 50–70 分钟（含代码和图表）
...
注意 `df` 显示的总容量 **14.2G** 小于 `/sys/block/mmcblk1/size` 对应的 **14.6 GiB**。这两个数字来自不同层面：

```
/sys/block/mmcblk1/size  (30576640 扇区 x 512 = 14.6 GiB)
  来源: eMMC EXT_CSD[212-215] SEC_COUNT
  含义: NAND 闪存物理总容量
  层面: 块设备层（硬件寄存器值）

df -h  (14.2G)
  来源: ext4 superblock 块组描述符 + 块位图统计
  含义: 文件系统总可用块数 x 块大小
  层面: 文件系统层（软件元数据）
```

差值原因有三个：
1. **ext4 元数据开销** — superblock、inode 表、块位图、journal 在格式化时已分配，不计入文件系统可用空间
2. **5% root 预留** — `mke2fs -m 5` 默认预留 5% 的块给 root 使用，`df` 的 Available 列已扣除
3. **单位进制差异** — `df` 用 1000 进制（G = 10^9），`size` 用 1024 进制（Gi = 2^30），14.2 GB ≈ 13.2 GiB，加上前两项刚好对上

所以"还剩多少空间"要分清楚问什么：

| 问题 | 看哪里 | 特点 |
|------|--------|------|
| 硬件总容量是多少 | `cat /sys/block/mmcblk1/size` | **固定不变**，是 eMMC 的物理属性 |
| 文件系统还能写多少 | `df -h` 的 Available 列 | 动态变化，数据删了会变多 |
