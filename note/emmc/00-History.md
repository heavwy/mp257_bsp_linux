...
| | MMC | SD | 后果 |
|--|-----|----|------|
| **引脚** | 7 pin | 9 pin（多了 2 个数据线） | SD 插槽能插 MMC，但反过来不行 |
| **加密** | 无 | CPRM（Content Protection for Recordable Media） | 音乐/视频行业愿意用 SD |
| **时钟** | 0-20MHz | 0-25MHz | SD 刷新率更高 |
...
UHS-I 的一个重要变化：**SD 卡的电压切换协议从 CMD0+CMD1 改为 CMD8（voltage switch）**。CMD8 检查卡是否支持 1.8V，然后通过 CMD11 切换。这套流程后来也被 eMMC 借鉴。
